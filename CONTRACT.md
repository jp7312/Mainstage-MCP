...
A tagged snapshot may commit only while its refresh request is pending. Expired replies cannot replace the last complete snapshot; a reply already expired when it begins also leaves freshness unchanged. Unsolicited snapshots retain an empty request id. Cancelling an explicit reconnect closes its owned helper, preserves stale cached state and propagates cancellation without replaying commands.
