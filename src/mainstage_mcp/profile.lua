...
local function escape(value)
    return (tostring(value):gsub('.', function(c)
        ...
        if byte < 32 or byte > 126 or c == '%' then return string.format('%%%02X', byte) end
        return c
    end))
end
...
function controller_initialize(applicationName, deviceNewlyDetected)
    ...
    if #application > 1024 then application = '' end
...
function controller_select_patch(program, patch, set, concert, list, setIndex, patchIndex)
    ...
    local events, keys, total = {}, {}, 0
    local function append(kind, fields)
        local event, key = packet(kind, fields)
        ...
        events[#events + 1], keys[#keys + 1] = event, key
        return true
    end
    ...
    for i = 1, count do
        ...
        if type(item) ~= 'table' or type(item.IsPatch) ~= 'boolean' or not index(item.SetIndex)
            or not index(item.PatchIndex) or not text(item.Label) then
    ...
    local key = table.concat(keys, '\n') -- Encoded fields cannot contain literal newlines.
...
if EXPERIMENTAL_PARAMETER then
    function controller_midi_out(midiEvent, name, valueString, color)
        ...
        local event = packet('parameter', {session, revision, parameterSequence + 1,
            ...
            scalar(name), scalar(valueString), 'screen_control_feedback'})
