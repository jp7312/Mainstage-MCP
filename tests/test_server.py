...
for line in sys.stdin:
    ...
    if c['command']=='cc' and c['control']==90:
        ...
        emit('parameter','fake-session',revision,sequence,'mapped_parameter_1',raw,0,127,
             'midi_7bit','Cutoff','1.2 kHz','screen_control_feedback')
    if c['command']=='refresh':
...
class ServerTests(unittest.IsolatedAsyncioTestCase):
    async def test_official_client_and_mutations(self):
        ...
        async with Client(create_server(bridge)) as client:
            ...
            self.assertTrue(cc['sent'])
            self.assertFalse(cc['observed'])
    ...
    async def test_atomic_session_and_malformed_feedback(self):
        ...
        state = b.snapshot()
        self.assertEqual(state.selection.concert,'A')
        self.assertTrue(state.stale)
        with self.assertRaises(ValueError):
            emit('snapshot_end','b',1,'wrong',0)
        ...
        self.assertEqual(b.snapshot().selection.concert,'B')
        with self.assertRaises(ValueError):
            emit('snapshot_begin','a',2,'')
        with self.assertRaises(ValueError):
            emit('snapshot_begin','b',0,'')
        self.assertTrue(b.snapshot().stale)
    ...
    async def test_delayed_selection_polls_without_resending_pc(self):
        ...
        fake = fake.replace("program=c['program']; revision+=1",
                            "pending=c['program']; polls=0; pc_count+=1\n"
    ...
    async def test_profile_capabilities_are_negotiated_strictly(self):
        ...
        for caps in (None, 'selection', 'selection,patch_list,raw_midi_cc,unknown',
            ...
            with self.assertRaises(ValueError):
                b.accept(dict(kind='hello',fields=fields))
            self.assertEqual(b.snapshot().capabilities, [])
    ...
    async def test_parameter_feedback_sequence_freshness_and_observation(self):
        ...
        async with Client(create_server(b)) as client:
            ...
            state = value(await client.call_tool('mainstage_refresh'))
            self.assertEqual(state['mapped_parameter']['observedAt'], observed_at,
                             'refresh replay is not a new observation')
    ...
    async def test_parameter_frames_are_strict_and_old_selection_is_stale(self):
        ...
        with self.assertRaisesRegex(ValueError, 'unsupported parameter mapping'):
            emit('parameter', 's', 1, 2, 'mapped_parameter_1', 64, 0, 127,
                 'dB', 'Cutoff', '-3 dB', 'screen_control_feedback')
            ...
            for case in ('absent', 'full', 'partial', 'cancel'):
                ...
                async with Client(create_server(b)) as client:
                    ...
                    if case == 'full':
                        ...
                        invalid = await client.call_tool('mainstage_toggle_metronome',
                                                         dict(args,expected_session='wrong'))
                        self.assertTrue(invalid.is_error)
                    ...
                    if case == 'absent':
                        ...
                        self.assertEqual(cc, [dict(control=80,value=127,channel=16),
                                              dict(control=80,value=0,channel=16)])
                        result = value(result)
                        self.assertFalse(result['observed'])
            ...
            for partial in (False, True):
                ...
                async with Client(create_server(b)) as client:
                    ...
                    if partial:
                        ...
                        self.assertEqual(result['messages_confirmed'], 1)
                        self.assertFalse(result['sent'])
                        self.assertTrue(result['may_have_changed_bank'])
                        ...
                        self.assertTrue(result['sent'])
                        self.assertFalse(result['observed'])
                        self.assertTrue(result['program_observed'])
                        self.assertFalse(result['bank_observed'])
